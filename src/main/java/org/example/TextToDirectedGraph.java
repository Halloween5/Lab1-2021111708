        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            for (int i = 0; i < words.length - 1; i++) {
                ...
                if (!currentWord.isEmpty() && !nextWord.isEmpty()) {
                    directedGraph.putIfAbsent(currentWord, new HashMap<>()); //指定的键不存在时，将键和值放入映射中
                    directedGraph.get(currentWord).put(nextWord, directedGraph.get(currentWord).getOrDefault(nextWord, 0) + 1); //去寻找映射中一模一样的项的个数，对权值加一。没有该项就默认为0，然后再加一。
                }
            }
        }
